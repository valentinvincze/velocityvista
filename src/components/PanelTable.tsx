...
export default function PanelTable({
  ...
  const table = profiles?.map((data) => {
    const coo = data.role === "coo";
    const admin = data.role === "admin";
    const rep = data.role === "rep";

    const rolePill = clsx("rounded-full px-2 py-1", {
      "bg-black text-white dark:bg-white dark:text-black": coo,
      "bg-gray-200 text-label dark:bg-neutral-600": admin,
      "bg-emerald-100 text-emerald-600 dark:bg-emerald-600/20 dark:text-emerald-400":
        rep,
    });
  ...
  const currentActiveStyle = clsx({
    "bg-black text-white dark:bg-white dark:text-black": current,
  });

  const nextActiveStyle = clsx({
    "bg-black text-white dark:bg-white dark:text-black": next,
  });
  ...
  const rightDisabledStyle = clsx({
    "cursor-not-allowed opacity-40": disabledRight,
  });
    ...
    <div className="flex-1 flex flex-col bg-card border border-gray-200 rounded-3xl dark:border-neutral-700 min-h-0">
      ...
      <div className="flex justify-between items-center p-5">
        <span className="text-label text-sm">
          {`${start + 1}-${Math.min(end, totalUserCount)} of ${totalUserCount} users`}
        </span>
